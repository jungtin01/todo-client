...
import { LABEL } from '@/commons/toast-notifications';
...
const getters = {
	getIndex: (state) => (id) => state.labels.map((label) => label.id).indexOf(id),
};

const mutations = {
	save(state, label) { state.labels.push(label) },
	update(state, { label, index }) { state.labels[index] = label },
	delete(state, index) { state.labels.splice(index, 1) },
	load(state, labels) { state.labels = labels },
};
...
const actions = {
	...
	async load({ commit }) {
		...
		const labels = res.data._embedded.labelList;
		commit('load', labels);
	},
};
